/** * iconupload.js * @authors Your Name (you@example.org) * @date    2016-03-18 18:10:41 * @version $Id$ */
$(document).ready(function (){
	...
	function checkfiles(files, name, type){
		...
		elseif(btype>-1 && filessize>size){
			alert("文件超出大小")
		}
		elseif(btype<0 && filessize>size){
			alert("请按规定上传")
		}
		else{
	...
	function canceluploda(slect){
		...
		,300)slectbox.find(".upload-loading").fadeOut('500');
	...
	function senfiles(slect, files, name, size){
		...
		xhr.upload.addEventListener("progress",function (evt){
			...
			upbox.find(".upload-prograssbox").css("opacity","1")upbox.find(".uploaded-pro").width(loadedwidth-20);
		});
