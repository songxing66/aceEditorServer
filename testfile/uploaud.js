...
$(document).ready(function() {
	...
	function checkchanger() { //检测当前优惠类型
	...
	aCheckAll.on("ifClicked", function() { //全选按钮事件
		allCheckBtn($(this));
	});
	$(".acheck").on("ifClicked", function() { //单选按钮事件
		aChecBtn($(this))
	});

	$(".tableinput").tagator(); //块标签关键词
	//协议弹窗
	...
	$(".keycomnum").keyup(function() {
		...
		if ((allTxtNum - endTxtNum) < 1) {
			textRemind.hide();
		} else if ((allTxtNum - endTxtNum) == 50) {
			textRemind.hide();
		} else {
	...
	$("input[data-interval]").on("change", function() {
		...
		} else if (iIntervalVal > iIntervalMax) {
	...
	$("input[data-reduce-check]").blur(function() {
		...
		var cost = iMonCardNum * 6; //6倍
		var full = iFullNum * 0.9; //90%
